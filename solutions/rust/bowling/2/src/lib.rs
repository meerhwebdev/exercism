    ...
    pub fn roll(&mut self, pins: u16) -> Result<(), Error> {
        ...
        if !last_frame.is_tenth {
            if pins == 10 {
                ...
                last_frame.rolls.push(pins);
                let acc_score = last_frame.rolls.iter().sum::<u16>();
                if acc_score > 10  {return Err(Error::NotEnoughPinsLeft)}
                last_frame.is_spare = acc_score == 10; 
                last_frame.is_completed = last_frame.rolls.len() == 2 || last_frame.is_spare
            }
        }else {
            ...
            if last_frame.is_strike {
                ...
                last_frame.rolls.push(pins);
                last_frame.is_completed = last_frame_rolls_len == 2
            } else if last_frame.is_spare {
                last_frame.rolls.push(pins);
                last_frame.is_completed = last_frame_rolls_len == 2
            }else {
                last_frame.rolls.push(pins);
                last_frame.is_strike = pins == 10;
                let acc_score = last_frame.rolls.iter().sum::<u16>();
                if acc_score > 10  {return Err(Error::NotEnoughPinsLeft)}
